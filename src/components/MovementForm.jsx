...
import { buildMovementDraftKey, clearMovementDraft, pickLatestMovementDraft, readIndexedMovementDraft, readLocalMovementDraft, saveMovementDraft } from '../services/draftStorage';
import { formatDateTime, nowDateTimeInputValue, nowIso, toDateTimeInputValue } from '../services/timeService';
...
export function MovementForm({
  ...
  const draftKey = useMemo(
    () =>
      buildMovementDraftKey({
        catalogVersion: settings?.catalogVersion || 0,
        operatorId: initialState.operatorId,
        equipmentId: initialState.equipmentId,
        activityTypeId: initialState.activityTypeId,
        recordId: record?.id || null,
      }),
    [initialState.activityTypeId, initialState.equipmentId, initialState.operatorId, record?.id, settings?.catalogVersion],
  );
  ...
  const hadDraftChangesRef = useRef(false);
  const hasDraftChanges =
    form.operatorId !== initialState.operatorId ||
    form.equipmentId !== initialState.equipmentId ||
    form.activityTypeId !== initialState.activityTypeId ||
    form.notes !== initialState.notes ||
    form.manualEntry !== initialState.manualEntry ||
    form.startDateTime !== initialState.startDateTime ||
    form.endDateTime !== initialState.endDateTime;

  useEffect(() => {
    let cancelled = false;

    const restoreDraft = async () => {
      const localDraft = readLocalMovementDraft(draftKey);
      const indexedDraft = await readIndexedMovementDraft(draftKey);

      if (cancelled) {
        return;
      }

      const resolvedDraft = pickLatestMovementDraft(localDraft, indexedDraft);

      if (resolvedDraft) {
        setForm((current) => ({
          ...current,
          ...resolvedDraft,
          operatorId: resolvedDraft.operatorId || initialState.operatorId,
          equipmentId: resolvedDraft.equipmentId || initialState.equipmentId,
          activityTypeId: resolvedDraft.activityTypeId || initialState.activityTypeId,
        }));
        setDraftState({ status: 'restored', savedAt: resolvedDraft.savedAt || '' });
        setErrors({});
        return;
      }
  ...
  useEffect(() => {
    ...
    if (!hasDraftChanges) {
      if (hadDraftChangesRef.current) {
        void clearMovementDraft(draftKey);
      }

      hadDraftChangesRef.current = false;
      setDraftState((current) => (current.status === '' && current.savedAt === '' ? current : { status: '', savedAt: '' }));
      return undefined;
    }

    hadDraftChangesRef.current = true;

    const timer = window.setTimeout(() => {
      const draftPayload = getDraftPayload();

      void saveMovementDraft(draftKey, draftPayload).then((persisted) => {
        if (persisted) {
  ...
  }, [draftKey, form.activityTypeId, form.endDateTime, form.equipmentId, form.manualEntry, form.notes, form.operatorId, form.startDateTime, hasDraftChanges, initialState.activityTypeId, initialState.equipmentId, initialState.endDateTime, initialState.manualEntry, initialState.notes, initialState.operatorId, initialState.startDateTime]);
  ...
  useEffect(() => {
    ...
    const flushDraft = () => {
      ...
      void saveMovementDraft(draftKey, getDraftPayload());
    };
  ...
  let draftMessage = t('movement.draftReady');

  if (draftState.status === 'restored') {
    draftMessage = t('movement.draftRestored');
  } else if (draftState.status === 'saved') {
    draftMessage = t('movement.draftSaved');
  } else if (draftState.status === 'error') {
    draftMessage = t('movement.draftError');
  }
